...
class MainProgram
{
	public static void main(String[] args) throws IOException
	{
		...
		int mutationppercentage=25;															//변이율 - 1이 0.1퍼센트임.
		...
		for (int i=0;i<generation-1 ;i++ ){
			...
			makingChild(parent, fitness, kmla, teacher, mutationppercentage, fatherGene,2);			//마지막 int 0: 룰렛휠 / 1: 우선순위기반 / 2: 토너먼트
			if (i==400)
				mutationppercentage = 15;
			if (i==1000)
				mutationppercentage = 1;
			if ((i+2)%500 == 0){
	...
	public static void firstRandomInitialize(Grade[][] kmla, Teacher[] teacher, int[][] parent, Fitness[] fitness){
			for (int i=0;i<parent.length ;i++ ){
	...
	public static void secondRandomInitialize(Grade[][] kmla, Teacher[] teacher, int[][] parent, Fitness[] fitness){
			for (int i=0;i<0.2*fitness.length ;i++ ){
				randomInitialize(kmla, teacher);
				int count=0;
				for (int j=0;j<kmla.length ;j++ )
					for (int k=0;k<kmla[j].length ;k++ )
